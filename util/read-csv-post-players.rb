...
CSV.foreach(infilename) do |playerrecord|
  ...
  player[:price]           = playerrecord[21].to_i + 200
  player[:team]            = playerrecord[3].to_i
  player[:bat_innings]     = playerrecord[4].to_i
  player[:bat_runs_scored] = playerrecord[5].to_i
  player[:bat_fifties]     = playerrecord[6].to_i
  player[:bat_hundreds]    = playerrecord[7].to_i
  player[:bat_ducks]       = playerrecord[8].to_i
  player[:bat_not_outs]    = playerrecord[9].to_i
  player[:bowl_overs]      = playerrecord[10].to_i
  player[:bowl_maidens]    = playerrecord[11].to_i
  player[:bowl_runs]       = playerrecord[12].to_i
  player[:bowl_wickets]       = playerrecord[13].to_i
  player[:bowl_4_wickets]       = playerrecord[14].to_i
  player[:bowl_6_wickets]       = playerrecord[15].to_i
  player[:field_catches]       = playerrecord[16].to_i
  player[:field_runouts]       = playerrecord[17].to_i
  player[:field_stumpings]       = playerrecord[18].to_i
  player[:field_drops]       = playerrecord[19].to_i
  player[:field_mom]       = playerrecord[20].to_i
